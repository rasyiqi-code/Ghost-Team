...
// ── hoisted mocks ──────────────────────────────────────────────
const { mockFindMany, mockFindFirst, mockInsertChain, mockUpdateChain, mockDeleteChain, mockSearchModels, mockGetModelFamilies, mockSearchProviders, mockGetProviderModels, mockListAvailableModels, } = vi.hoisted(() => {
...
vi.mock('../../core/encryption.js', () => {
    const mockEncrypt = vi.fn((s) => `encrypted:${s}`);
    const mockDecrypt = vi.fn((s) => s.startsWith('encrypted:') ? s.slice('encrypted:'.length) : s);
