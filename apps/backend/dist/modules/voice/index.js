...
let socketIO;
export async function voiceModule(app) {
    socketIO = app.io;
...
async function processVoiceNote(userId, messageId, audioPath) {
    try {
        ...
        if (summary)
            display = `📝 ${summary}\n\n${rawText}`;
        const taskList = tasks?.daftar_tugas ?? [];
        if (taskList.length > 0) {
            const taskLines = taskList.map(t => `  • [${t.prioritas ?? '?'}] ${t.divisi ?? 'general'}: ${t.deskripsi ?? ''}`);
            display += '\n\n📋 Tugas:\n' + taskLines.join('\n');
        }
        await db.message.update({
            where: { id: messageId },
        ...
        await db.message.update({
            where: { id: messageId },
            data: { content: `[Voice note processing failed: ${err.message}]`, messageType: 'voice_processed' },
        }).catch(() => { });
    }
}
